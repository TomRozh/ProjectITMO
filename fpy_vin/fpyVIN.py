...
from serial_port import serial_portPY as serial_portPy
...
class VINMachine:
    ...
    def __init__(self, args, port=None, it_is_binary=False):
        if it_is_binary == False:
            self.args = args
            if port is not None:
                self.serial_port = serial_portPy.SerialPort(str(port), timeout=self.args.timeout)
                self.start()
        else:
            self.args = args
            if port is not None:
                self.serial_port = serial_portPy.SerialPort(port, timeout=100)
                self.start()
    ...
    def reset_input_buffer(self): #-----------
        if serial_portPy.SerialPort.opened:
            serial_portPy.SerialPort.reset_input_buffer(serial_portPy.SerialPort)
    ...
    def command_send(self, address, cmd, bin_command=None, data=None, it_is_binary=False):
        if not serial_portPy.SerialPort.opened: #------
            print('No connection set on device port')
            return None
        try:
            serial_portPy.SerialPort.flush
        except termios.error:
            print('I/O Error. device has been disconected.')
            return None
        if it_is_binary == False:
            msg = self.compose_write(self,address, cmd, data)
            if self.send_callback is not None:
                self.send_callback(msg)
            return serial_portPy.SerialPort.write(serial_port.serial_portPY.SerialPort,msg) #--------2
        else:
    ...
    def reply_read(self):
        ...
        while not got_frame:
            if not serial_portPy.SerialPort.opened:
                serial_portPy.SerialPort.open()
                return [None, '']
            try:
                data = serial_portPy.SerialPort.read(serial_port.serial_portPY.SerialPort,1).decode('latin-1')
            except serial.serialutil.SerialException:
                if len(serial_portPy.scanAvaiableSerialPorts()) != 0:
                    port = sorted(serial_portPy.scanAvaiableSerialPorts())[0]
                    self.serial_port = serial_portPy.SerialPort(str(port), timeout=self.args.timeout)
    ...
    def command_a(self, address, cmd, data=None, it_is_binary=False, bin_command=None):
        ...
        self.reset_input_buffer(self)
        self.command_send(self,address=address, cmd=cmd, data=data, it_is_binary=it_is_binary, bin_command=bin_command)
        if it_is_binary == False:
            if address == BROADCAST_ADDRESS:
                return self.command_c(self, address, cmd, data)
            return self.reply_read(self)
        else:
